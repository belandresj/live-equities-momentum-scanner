...
function fixture(name) {
  const snapshot = snapshotFixtureV2(name === "empty" ? 0 : 20);
  ...
  if (name === "degraded") {
    ...
    snapshot.accounting.population_transition_diagnostic.bootstrap_unknown = 1; snapshot.accounting.population_transition_diagnostic.no_later_eligible_mark = 1;
  ...
  if (name === "tq") {
    ...
    for (const [index, row] of snapshot.rows.entries()) {
      row.tape_5s = { status: "pressure_shed", reason: "pressure", trade_coverage: false, trades_per_second: null, timestamp_basis: "", lifecycle_records_observed: row.tape_5s.lifecycle_records_observed };
  ...
  if (name === "hostile" && snapshot.rows.length) {
    ...
    snapshot.rows[0].activity_30s = { status: "unavailable", reason: "history_incomplete", value_ratio: null };
  }
...
const server = http.createServer((request, response) => {
  ...
  if (request.method !== "GET" || request.url !== "/api/v2/snapshot") { response.writeHead(404).end(); return; }
