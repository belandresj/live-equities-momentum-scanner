...
function fieldView(field, stops) { return knownCurrentMeasurement(field) ? { state: "current", text: formatPercent(field.value_ratio), reason: field.reason, band: band(field.value_ratio * 100, stops) } : { state: KNOWN_FIELD_STATUS.has(field.status) && KNOWN_FIELD_REASON.has(field.reason) ? field.status : "unknown", text: "—", reason: field.reason || field.status, band: 0 }; }
...
export function buildViewModel(input, transport = "connected") {
  ...
  const rows = snapshot.rows.map(row => {
    ...
    return {
    rank: row.rank, symbol: row.symbol, last: formatUSD(row.last_usd), day: formatPercent(row.day_change_ratio), dayBand: band(row.day_change_ratio * 100, [0, 1, 2.5, 5, 10]), markAgeMS: row.mark_age_ms,
    from4am: fieldView(row.from_4am_change, [0, 1, 2.5, 5, 10]), hod: fieldView(row.hod_drawdown, [0, 1, 2.5, 5, 10]), dayRange: fieldView(row.day_range_position, [0, 25, 50, 75, 100]), range60: fieldView(row.range_60m_position, [0, 25, 50, 75, 100]), range30: fieldView(row.range_30m_position, [0, 25, 50, 75, 100]), activity: fieldView(row.activity, [0, 25, 50, 75, 100]),
    tape: { state: tqState(row.tape_rate.status, row.tape_rate.reason), band: tapeCurrent ? band(row.tape_rate.five_second.trades_per_second, [0, 1, 5, 15, 30]) : 0, primary: rateView(row.tape_rate.five_second, tapeCurrent), secondary: rateView(row.tape_rate.one_second, tapeCurrent), detail: `status ${row.tape_rate.status}; reason ${row.tape_rate.reason || "none"}; coverage ${row.tape_rate.trade_coverage ? "yes" : "no"}; timestamp ${row.tape_rate.timestamp_basis || "none"}; lifecycle records ${row.tape_rate.lifecycle_records_observed ? "observed" : "not observed"}; membership desired ${row.tq_membership.desired ? "yes" : "no"}, provider ${row.tq_membership.provider_present ? "present" : "absent"}, unknown ${row.tq_membership.provider_membership_unknown ? "yes" : "no"}` },
    spread: { state: tqState(row.spread.status, row.spread.reason), band: knownCurrentTQ(row.spread.status, row.spread.reason) ? band(row.spread.basis_points, [0, 5, 10, 25, 50]) : 0, primary: knownCurrentTQ(row.spread.status, row.spread.reason) ? `${row.spread.basis_points.toFixed(1)} bps` : "—", secondary: knownCurrentTQ(row.spread.status, row.spread.reason) ? `${row.spread.cents.toFixed(2)}¢` : "", detail: `status ${row.spread.status}; reason ${row.spread.reason || "none"}; coverage ${row.spread.quote_coverage ? "yes" : "no"}; duration ${row.spread.valid_duration_ms} ms; quality ${row.spread.quality || "none"}; membership desired ${row.tq_membership.desired ? "yes" : "no"}, provider ${row.tq_membership.provider_present ? "present" : "absent"}, unknown ${row.tq_membership.provider_membership_unknown ? "yes" : "no"}` },
  }; });
  return {
