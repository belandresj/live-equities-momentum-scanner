...
export function buildViewModel(input, transport = "connected") {
  ...
  const rows = snapshot.rows.map(row => {
    ...
    from4am: fieldView(row.from_4am_change), hod: fieldView(row.hod_drawdown), dayRange: rangeFieldView(row.day_range_position), range60: rangeFieldView(row.range_60m_position), range30: rangeFieldView(row.range_30m_position), activity: fieldView(row.activity, [0, 25, 50, 75, 100]),
