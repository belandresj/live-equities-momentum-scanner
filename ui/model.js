...
function fieldView(field) { return knownCurrentMeasurement(field) ? { state: "current", text: formatPercent(field.value_ratio), reason: field.reason } : { state: KNOWN_FIELD_STATUS.has(field.status) && KNOWN_FIELD_REASON.has(field.reason) ? field.status : "unknown", text: "—", reason: field.reason || field.status }; }
