...
func TestEvaluatorIntegrityInspectionIsDetached(t *testing.T) {
	...
	capture := SnapshotCapture{sealed: &sealedSnapshotCapture{view: SnapshotCaptureView{
		...
		Status:  Status{IntegrityFailure: failure},
		Metrics: Metrics{Engine: engine.OperationalView{IntegrityFailure: failure}},
	}}}
	first, ok := InspectSnapshotCapture(capture)
	if !ok {
		t.Fatal("sealed diagnostic capture was unavailable")
	}
	...
	if !ok || second.Engine.Operational.IntegrityFailure.Category != engine.EvaluatorSupportContradiction ||
		second.Status.IntegrityFailure.Category != engine.EvaluatorSupportContradiction || second.Metrics.Engine.IntegrityFailure.Category != engine.EvaluatorSupportContradiction {
