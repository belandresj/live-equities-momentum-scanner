...
tail-presence index. Each derived bitmap word has an independently maintained
complement guard, and canonical-tail revision, indexed revision, and identity
count must agree before the derived word may prove coverage; mismatch fails
closed. Ordinary in-order insertion, revision, withdrawal, and one-second
expiry update that index incrementally; a correction may perform bounded
symbol-local reordering. The hydration terminal may build it once from the
bounded tail. A routine one-second cycle may visit newly expired identities but
may not allocate a tail-sized work list, rescan all retained identities, or
...
| `LBR-R2` | `P-LBR-R2-INCREMENTAL-MAINTENANCE` | A focused canonical trace covers out-of-order insertion, revision, withdrawal, strict/equality expiry, folded-prefix plus tail coverage, malformed rebuild, same-cardinality stale-index revision, and bitmap-only stale-content counterexamples. Existing accepted retained-tail, aggregate-coverage, qualification, and direct historical-hydration compaction regressions remain participating semantic evidence. A separate allocated measurement gives 5,554 symbols independent full-cardinality 961-identity canonical maps, expiry-order storage, guarded presence bitmaps; immutable record objects are shared only by the read-only timing fixture. It requires zero cycle allocation and latency below the parent 250 ms maximum on the current host. It detects stale derived coverage through unmatched canonical/index revisions or a broken bitmap-word guard, skipped expiry, routine tail-sized work, or a population-sized allocation. It does not compare complete mature projected state to a separate scan oracle and does not establish E2 capacity or provider stability. |
