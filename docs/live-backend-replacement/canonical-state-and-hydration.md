...
terminal, and fence outcomes are retained; their representations, ledgers,
checkpoint/replay paths, worker topology, and accepted implementation are not.
No predecessor checkout or provider access is part of this contract.
...
fresh hydration `[S,R)` or exact gap recovery. There is one bounded REST worker.
The worker owns request construction, at most two pages, at most three bounded
attempts per page, the existing page/row limits, normalization, and one
terminal result per request. It has no access to `SymbolState`.
...
API/UI changes, launcher changes, public deployment, replay repair, and
checkpoint repair are non-scope. Slice evidence and status are recorded only
