...
authoritative; if this process or a focused specification appears to conflict
with them, stop and resolve the conflict with the owner.

## 1. Contract-first rules

Before drafting a focused component specification, the assigned agent must:
...
6. create a short Phase 1 contract skeleton without inspecting version 2; the
   skeleton states the outcome, controlling IDs, ownership/non-scope, settled
   semantic boundary and invariants, unresolved evidence questions, and the
   proposed version 2 reconnaissance scope;
...
8. after approval, inspect only the approved relevant version 2 sources and use
   the findings to complete the focused component specification;
9. define one bounded implementation slice or a sequential slice plan that
   assigns every requirement and primary proof exactly once; and
10. stop for owner contract/reuse/test/slice-plan approval before an
...
focused spec cites their exact requirement IDs and defines only its own
...
approved Phase 1 requirement(s)
  -> short Phase 1 contract skeleton drafted without version 2
  -> owner boundary and reconnaissance-scope approval
  -> narrow version 2 code/test/fixture reconnaissance
  -> completed focused contract and reuse/proof assessment
  -> owner contract/reuse/test/slice-plan approval
  -> authorize one bounded implementation slice
  -> implement slice and run its allocated primary proof
  -> owner slice review
  -> authorize the next slice or complete the component
...
not authorize every slice at once. Code starts only when the owner authorizes
the first bounded slice assignment, and each later slice waits for review of
the preceding slice.
...
Use
[`specifications/focused-component-spec-template.md`](specifications/focused-component-spec-template.md).
...
reuse assessment to the focused spec for each candidate containing:
...
After reconnaissance, complete the focused specification. It defines semantic
...
Proof planning occurs in the focused spec before implementation. Each
requirement receives one primary proof. A second test at another layer is
allowed only when it proves a distinct boundary, and the spec names that
...
Before owner approval, decide whether the component is small enough for one
bounded assignment. Split it when it contains multiple independently observable
behaviors, distinct proof families, provider I/O plus canonical mutation,
separately provable parts, multiple ownership/package boundaries, or a change
set that would be difficult to review as one unit. Do not split by estimated
hours, lines, or token count. Split at behavior and proof boundaries.
...
any intentionally deferred secondary evidence.
...
9. required verification and owner-review artifact;
...
- `AGENTS.md`;
- the approved focused component spec;
...
### 2.6 Slice implementation, owner review, and component review
...
At the end of every slice, stop and provide an owner-review handoff containing:
...
- the requested decision: accept, revise, split, or stop.

The next slice requires explicit owner authorization. A failing proof or an
implementation discovery may require revising the component spec or remaining
slice plan; it does not authorize changing the contract inside the current
assignment.

After all slices pass, final component review verifies:
...
specs. This process does not maintain a second list. The map covers reference
...
slice. A component with several slices completes them in approved order, with
owner review between slices. While component N is being implemented, the
contract skeleton, bounded version 2 reconnaissance, and detailed specification
for component N+1 may advance. Do not grant final N+1
contract/reuse/test/slice-plan approval or authorize N+1 implementation until
all N slices have passed their allocated primary proofs, final component
review, and any milestone boundary that the map names as a dependency. The
owner may approve an earlier N+1 contract decision only when it depends
exclusively on an already approved stable interface and the exception is
recorded explicitly.

This one-component lookahead keeps specification work moving without allowing
two implementations to redefine a shared interface concurrently. Slice review
also prevents one agent from carrying an oversized component through repeated
context compaction or unreviewed scope expansion. If component N reveals that
an approved N+1 skeleton assumption is wrong, revise that skeleton before
completing or approving its detailed contract; do not reopen completed
component N unless an authoritative requirement was missed.
...
The resolution belongs in the applicable focused component spec if it changes
