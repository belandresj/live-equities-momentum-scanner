...
| [`live-tq-resilience-correction.md`](live-tq-resilience-correction.md) | Current owner-requested C5/C8/C9 correction: accept asynchronous provider-status framing without letting T/Q-local ambiguity suppress aggregate ranking, quarantine contaminated T/Q control, route actual aggregate loss through exact recovery, and keep the process/API alive under explicit bounded same-binding recovery. Two sequential slices precede the broader live-scanner correction; no live request is authorized. |
| [`live-scanner-recovery-narrow-fix.md`](live-scanner-recovery-narrow-fix.md) | Queued owner-requested correction specification after the T/Q resilience correction: compose live aggregate coalescing with the corrected explicit-recovery guard, measure and correct only the late-session Activity hotspot, and require sub-two-minute proofs before any longer cached-data or live run. The earlier bootstrap and incremental-evaluator documents remain evidence, not competing executable plans. |
| [`replay-cli-event-time-correction.md`](replay-cli-event-time-correction.md) | Owner-approved 2026-08-12 C12 correction: finite CLI-only event-time replay, hidden unpaced warm-up, unpaced selected-window output, no HTTP/UI/live hydration, group-boundary complete-artifact evaluation, and bounded retained-day acceptance. |
| [`replay-cli-event-time-goal.md`](replay-cli-event-time-goal.md) | Current C12 implementation handoff: isolate the dirty worktree, record the correction, implement and reaccept implicated C2-C4 mechanics sequentially, and accept the finite unpaced CLI from compact and retained-day proofs plus conformance. |
...
| [`c12-implementation-goal.md`](c12-implementation-goal.md) | Superseded 2026-08-12 handoff retained as history for accepted C4-S6 and prior C12-S1 evidence. Do not use it to start C12-S2 or the former 1x/API/UI plan. |
...
containment/fallback rules. Review is exceptional and risk-triggered; component
completion, reopening, correction, and green deterministic verification do not
create an automatic review gate.
...
| 4 | [Aggregate replay](specifications/aggregate-replay.md) | Finally accepted through `C4-S6` on 2026-08-09. Bounded candidate/validation/playback cancellation, source-owned idempotent manual shutdown, retained terminal ordering, exact accounting, and the honest one-shot Run/error seam passed proof, race, review/correction/re-review, and conformance. Current `C12R` consumes this boundary and may reopen only the persisted playback-construction claim for a proved redundant scan. Owns offline downloader/compiler, normalized artifact, deterministic source/clock, shared aggregate path, and full-source-trusted prefix completion with distinct lifecycle/accounting. |
...
| 12 | [Historical replay product mode](specifications/historical-replay-product-mode.md) | Reopened 2026-08-12 by the [CLI event-time replay correction](replay-cli-event-time-correction.md). Prior C12-S1 is evidence; mandatory 1x/API/dashboard behavior and S2 are superseded. Current outcome is a finite, fully unpaced CLI replay through the sole C4 source and `ScannerStateEngine`, accepted only after compact equivalence/trust proofs and the retained-day bounded run. C12 remains outside the C7-C11 V1 RC gate. |
