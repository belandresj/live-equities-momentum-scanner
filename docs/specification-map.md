...
| 4 | [Aggregate replay](specifications/aggregate-replay.md) | Reopened narrowly 2026-08-11 for measured production-reader allocation mechanics under `C4-ART-02`; accepted schema/trust/lifecycle semantics, `C4-S1`--`C4-S6`, and their proofs remain preserved. The correction may remove only demonstrated redundant canonical validation allocation and requires focused persisted-trust proof/review before re-acceptance. Owns offline downloader/compiler, normalized artifact, deterministic source/clock, shared aggregate path, and full-source-trusted prefix completion with distinct lifecycle/accounting. |
