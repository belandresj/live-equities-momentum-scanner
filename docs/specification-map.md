...
| 4 | [Aggregate replay](specifications/aggregate-replay.md) | Finally accepted 2026-08-06. Owns offline downloader/compiler, normalized artifact, deterministic source/clock, and shared aggregate path. |
...
| 7 | [Checkpoints and restart](specifications/checkpoints-and-restart.md) | V1 contract revised 2026-08-07. S1/S2 evidence is accepted and reopenable; preserved S3 correction work is active. The inherited five-second load/install gate is removed; coherent end-to-end recovery remains hard. |
| 8 | [Readiness and operations](specifications/readiness-and-operations.md) | Owner-approved V1 boundary/reconnaissance plan. Detailed contract waits for C7 final acceptance. Runnable composition, readiness/staleness, bounded recovery/shutdown, measurements, and controlled 6,000-symbol mixed load. |
| 9 | [Top-20 T/Q coverage and features](specifications/top-20-tq-coverage-and-features.md) | Owner-approved V1 boundary/reconnaissance plan. Detailed contract waits for C8. Continuous selected coverage, Tape Rate, NBBO Spread, aggregate-first shedding to zero, current-rank restoration. |
| 10 | [Versioned snapshot API](specifications/versioned-snapshot-api.md) | Owner-approved V1 boundary/reconnaissance plan. Detailed contract waits for C9. Private loopback read-only HTTP schema, coherent snapshots, publication identity, field status, explicit CORS allow-list. |
| 11 | [Independent UI](specifications/independent-ui.md) | Owner-approved V1 boundary/reconnaissance plan. Detailed contract waits for C10. Independent Chrome-desktop high-fidelity V2 adaptation with every V1 field/status and no browser-owned market logic. |
