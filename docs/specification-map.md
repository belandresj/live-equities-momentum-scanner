...
| 9 | [Top-20 T/Q coverage and features](specifications/top-20-tq-coverage-and-features.md) | Finally reaccepted 2026-08-13 after the owner-directed recovery delivery gate changed from `<500 ms` to an attributed maximum `<1 s`. Exact below/equal, delayed, and unattributed proofs pass while 2-second degraded, 5-second aggregate-only, queue/heap/goroutine, 30-second dwell, restoration, and aggregate-independence behavior remain unchanged; live/provider capacity remains unclaimed. |
