...
| Live T/Q continuity correction | `accepted_on_clean_main` | [`live-tq-resilience-correction.md`](../live-tq-resilience-correction.md) partitions T/Q accounting from aggregate/control accounting, polls quiet sockets for engine lifecycle changes, keeps `same_binding_recovery_allowed` joined behind one engine-issued bounded command, and routes representable aggregate loss to exact gap recovery. Session end and restart-required suppression remain terminal. Focused runtime/API proofs and repository short pass. | Complete deterministically; owner-run live confirmation pending. |
| Owner-directed daily hydration concurrency | `accepted_lower_level_revision` | The 2026-08-13 owner run used the scanner's already valid eight-worker path, completed hydration, and reached advancing `qualified_current` output with 20 ranked rows. The supervised launcher now defaults to eight and exposes only the scanner's existing closed set `1|2|4|8`; shell and Go validation reject missing, repeated, or unsupported values before build/preflight/credential access. The value changes only C6 worker concurrency and the existing worker-scaled resident-record bound. | Complete locally; no new capacity/SLA claim, provider rule, readiness predicate, or market semantic. |
...
| Private daily operational finalization | `accepted_after_owner_revision` | `./scripts/run-private-scanner` supplies an eight-worker default plus validated `1|2|4|8` override and the same persistent-path workflow, isolates credentials, gates dashboard startup on `/livez`, reports engine `/readyz`, and contains both processes. Public-wrapper/fake-process tests reject invalid worker values before build or credential access. | Complete locally; prior two-worker evidence remains historical and no eight-worker capacity/SLA claim is inferred |
...
full-population hydration plan with eight workers by default in the supported
launcher (one, two, four, or eight are valid explicit values), at most two pages and three
