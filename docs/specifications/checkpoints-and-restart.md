...
| Checkpoint hot-path correction | `reaccepted` | The temporal overwrite claim is corrected: projection now seals one bounded value record per symbol at `T0`, so later timer evaluation cannot replace evidence for an unprojected symbol. The two-symbol regression advances the ordinary evaluator to `T0+2s`, persists/reopens/decodes/installs the original image, and regenerates the original `T0` evaluation. The revised 6,000-symbol proof advances `T` twice during each active projection; seal allocation is 1,884,968 bytes with an 816,000-byte marker-value slice, seal time is 0.191–1.141ms, maximum combined seal/continuation hold is 64.449ms, aggregate delivery is at most 369.719ms, observer delay is at most 399.507ms, and post-GC heap is stable. Prior semantic, ownership, codec/store, invalid-artifact, atomic-install, and restart evidence remains accepted. See [`live-checkpoint-hot-path-correction.md`](../live-checkpoint-hot-path-correction.md). | 2026-08-13 | Complete; separately authorized provider validation remains pending |
