...
| `internal/scanner/state.go`: `activityComponents`, `recomputeActivityBlock`, `passesActivityReferenceEligibility`, `populateActivity`, `empiricalPercentile` | state hash above | Establishes session-aligned 30-second blocks, ATS invalidity, an inclusive 100-transaction reference floor, inclusive ties, correction insert/remove/replace, and session-to-date retention. Its five-second maintenance delay remains incompatible. | Adapt arithmetic, eligibility, and retention; reject delay | Apply the exact session boundary and bounds below; prove target/reference and correction equivalence. |
| `internal/scanner/activity_product_state_test.go`: all eleven focused Activity tests | `9c06b2e48fdb9dc60723764afc84c61d8164a83723cd7762fd3139212bbec960` | Strong arithmetic, 10-reference, tie, correction, independence, session-to-date, and field-local bound evidence. The five-second-delay expectation conflicts with the current engine. | Behavior evidence, with the delay case rejected | Reuse only named arithmetic/tie/correction/session/TQ-independence inputs; replace delay expectations. |
...
| Activity state | Session-aligned eligible 30-second reference summaries, mutable-block state, and a separate Activity-only folded-target projection retaining identity-positioned `(Volume/ATS, High, Low, validity)` contributions. | At most 1,920 aligned summaries, 33 mutable block identifiers, 1,920 evaluation-reference capacity, 1,920 lazily allocated folded-target blocks, and 57,600 folded target contributions per printed symbol; no canonical raw bar copy. |
...
inside `[S,T-30s)`, its coverage is trustworthy, it contains at
...
The baseline always begins at `S`, the 04:00 New York session boundary, and
does not reset at 09:30 or 16:00. There are at most 1,919 eligible reference
positions at one in-session `T`.
...
retains same-session references through the binding. Aggregate or timer
transitions remove only references outside the bound session. A correction may
insert, replace, or remove an eligible reference.
...
uses at most 1,919 prior reference positions at an in-session `T`.
...
1,920 Activity block summaries, 33 mutable Activity-block identifiers, 1,920
ephemeral Activity reference capacity, 1,920 folded-target blocks, and 57,600
...
| `C3-ACT-01` | Target/reference table plus exact-coverage and non-aligned folded-target amendments | Exact statistics and statuses; counters unknown intervals as absence, an hour/session-boundary reset, target reuse, or a folded real target aggregate becoming `no_aggregate_in_target`. | Includes exact `no_aggregate_in_target`, a 13-hour premarket-through-after-hours session-to-date discriminator, and full-history-oracle equality after folded catch-up; Component 6 facts and provider ATS parity remain unproved. | `C3-S2`; `C3-R1`; `C3-R2` |
| `C3-ACT-02` | Activity correction, stalled-`T`, session-long, and folded non-aligned catch-up trace | Equivalence within 1,920 retained summaries, 33 mutable IDs, 1,920 evaluation-reference capacity, 1,920 folded-target blocks, and 57,600 folded target contributions; counters same-session expiry or aligned-only compaction destroying catch-up evidence. | Canonical differential output, old-`T` stability, strict raw-tail fold, later non-aligned target equality, permitted correction/withdrawal containment, retained-state ceilings, and no rank mutation. Checkpoint codec/capacity remain unproved. | `C3-S2`; `C3-R1`; `C3-R2` |
