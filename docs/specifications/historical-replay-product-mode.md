...
| `C12-S2` dashboard/real-artifact acceptance | `correction_required` | The replay presentation has deterministic coverage, but the first in-place B4 product attempt on 2026-08-13 exposed a real scale failure before observation: complete-artifact validation and exact binding succeeded, the loopback API entered `warming`, and controlled cancellation joined cleanly, but unpaced warm-up advanced only 107 of 20,101 groups in about 107 seconds. The observed rate projects roughly 5.5 hours to 09:30, so `P-C12-B4` cannot finish within its 15-minute hard bound. No artifact copy, credential access, provider request, or live-process stop occurred. | 2026-08-13 | Correct warm-up throughput without changing logical order, engine ownership, timers, or observation pace; rerun the narrowest scale discriminator before the full bounded `P-C12-B4` and Chrome acceptance |
