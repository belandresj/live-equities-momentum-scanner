...
**Allocated slices:** accepted `C4-S1`–`C4-S6`
...
| `C4-S6` | A candidate artifact date can be read without becoming trusted, every consequential artifact scan is context-cancelable, and a manual source has one bounded idempotent C4-owned cancel/join operation. | `C4-BOUNDED-CANCEL-01`; `P-C4-BOUNDED-CANCEL`. | Accepted S1–S5, completed C11/private V1 RC, and owner approval recorded 2026-08-09. Accepted before C12-S1. | `internal/replayartifact` candidate probe/validation, `internal/replayartifact/playback`, `internal/replay`, focused tests, and—after the recorded correction-loop scope revision—the existing Component 4 `cmd/aggregate-replay` Run/Cancel consumer and its tests only. Minimum compatibility wrappers are allowed. No C12/C8/C10/C11 code or engine-semantic change. | None. | Full cancellation-point and terminal-link matrix, exact accounting/deadline results, focused ordinary tests, repository short suite, proportionate race, and required persisted-source/lifecycle read-only review with clean re-reviews after correction. | C12 phase/schedule/API/UI composition, B4 artifact execution, provider/credential access, licensed rows, V2 inspection, capacity or product claims. |

**C4-S6 exact implementation assignment (owner-approved; activated after C11/V1
...
non-scope is C12 scheduling/phases/API/UI, provider activity, and B4 execution.
The allowed packages and empty V2 whitelist are the S6 table row, including the
recorded minimum C4 command-consumer correction needed to preserve the one-shot
terminal-result contract without bypassing caller deadlines. The sole
primary proof is `P-C4-BOUNDED-CANCEL`; use small repository-owned fixtures,
bounded waits, ordinary verification under two minutes, focused race under an
explicit timeout, and the required read-only review. The dangerous cases are
premature trusted success, unbounded cancellation, double terminal outcome,
and a second engine shutdown owner. Record exact results in the parent ledger;
activation records Component 4 reopened for only this claim while preserving
S1-S5 evidence, and acceptance requires clean proof, verification, review,
re-review after correction, and conformance before returning the component to
accepted. All C12 behavior remains deferred. Any need to change artifact identity,
terminal-fact meaning, engine ownership, or the fixed Phase 1 lifecycle stops
for contract correction rather than being absorbed into S6.
...
- `C4-S6` requires a narrow read-only persisted-source/
  ...
  was introduced, plus focused clean re-review after any correction.
...
- [x] `C4-S6` has separate owner approval and an exact assignment.
- [x] `C4-S6` implementation, proof, verification, focused review/corrections/
      re-reviews, and parent-ledger acceptance are complete. C12-S1 may now
      consume the accepted boundary.
