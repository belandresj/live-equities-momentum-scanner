...
| Owner-selected live-retry headroom | `accepted_after_correction_review` | The 2026-08-12 owner directed removal of unnecessarily tight delivery caps before another live run. The raw FIFO ceiling/production selection is revised from 8,192 to 32,768 descriptors while retaining the independent 128-MiB total-payload and 8-MiB per-frame limits, exact FIFO/accounting, and fail-closed saturation. Review exposed quadratic slice dequeue; the fixed-size ring correction now passes exact full-ceiling wrap/saturation/drain race, and end-to-end 8,192-slot capacity control/failure passes five race repetitions. This adds transient burst duration without permitting unbounded memory or changing normalization, engine ownership, readiness, or market semantics. Final `gpt-5.6-sol` medium re-review returned CLEAN/PASS with no remaining P1/P2. | 2026-08-12 | Complete locally; full 32,768 Runtime/provider composition remains unclaimed. |
| Live T/Q protocol resilience correction | `accepted_on_clean_main` | [`live-tq-resilience-correction.md`](../live-tq-resilience-correction.md) replaces frame-local status completion with bounded cumulative correlation, preserves interleaved causal elements, classifies explicit unsupported event families as bounded drops, and emits typed T/Q quarantine rather than aggregate ingress failure for post-handshake status contamination. Focused adapter proofs and repository short pass; no credentialed provider request was made. | 2026-08-14 | Complete deterministically; owner-run live confirmation pending. |
