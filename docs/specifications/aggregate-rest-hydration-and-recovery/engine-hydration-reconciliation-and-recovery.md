...
| `C6-MERGE-01` | Route every historical row through Component 2's existing historical fill-only merge under its exact token/result context. A REST row differing from valid live authority is counted as a bounded discrepancy while the live identity and request coverage remain exact; unresolved historical/historical conflict retains the accepted fail-closed consequence. Component 6 records that disposition without a second value map or evaluator. Provider `completed_value` does not by itself prove usable coverage when any row has an unresolved rejection/conflict. | `DTE-MERGE-01`–`05`, `DTE-HYDRATE-02`, accepted `ENG-AGG-01`; V2 conflict/stale interleavings and the 2026-08-12 live-precedence correction. |
...
| Historical canonical consequence | Existing Component 2 acceptance returns an exact disposition for every row; result context retains live authority and unresolved ambiguity through terminalization. | A discrepancy resolved by valid live precedence remains diagnostic-only. Historical/historical ambiguity or another unresolved rejection marks only the dependent interval/field unknown unless a global invariant fails. | Provider success being equated to canonical coverage despite an unresolved rejected/conflicting row, or a resolved REST/live discrepancy poisoning valid live coverage. |
