...
slice. The accepted correction has been transplanted onto the clean mainline
that already contains [`live-scanner-recovery-narrow-fix.md`](live-scanner-recovery-narrow-fix.md).
C12 remains paused. No credentialed provider request is authorized.

**Mainline integration note:** Mainline decision D8 supersedes this
correction's earlier experimental pressure constants. The integrated behavior
keeps the 32,768-frame queue, one-second sampling, T/Q degradation at 10% for
two samples or one-second oldest-frame age, and aggregate-only containment at
25% for three samples or two-second oldest-frame age. The T/Q quarantine,
accounting partition, exact aggregate recovery, and scheduled continuation
...
value, and no tight loop. The accepted deterministic burst must cross the
mainline early-shed boundary while remaining below the 32,768-frame hard limit.
...
The integrated mainline pressure policy samples once per second. Two samples
at 10% queue occupancy enter `taq_degraded`; three samples at 25% enter
`aggregate_only`, with the documented oldest-frame alternatives. The hard
queue limit is 32,768 frames. The paired proofs admit a 25%-boundary burst
without a capacity rejection and separately prove T/Q shedding at 12.5%, below
both quarter capacity and the hard limit. They preserve mixed-frame
aggregate/control classification and do not claim a host-saturation or
sampler-versus-burst race frontier.
...
`TestPTQRPressureShedsBeforeQuarterQueueAndHardCapacity`, and
...
The original correction's allocated medium-reasoning read-only review found that the first live-loop
...
The clean-main transplant reran the named focused proofs and the repository-wide
short suite successfully; the earlier dirty-worktree failures recorded under
S1/S2 are not present on this branch.
